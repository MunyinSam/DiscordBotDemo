# DiscordBotDemo
I am not permitted to release the original version of the server bot since it's a private repository.
So this a copy demo version containing only parts of the main code 
Requirements
- A MongoDB database [Not provided]
- Discord Bot Token (Available in Discord Developer Portal) [Not Provided]

# Functions

- Stores member data (stats,coins,items)
- Displays Market stocks
- Buy Items
- Sell Items
- Spawn Monsters (different in every zones)
- Combat System
- Reward System

# Real Repository

- https://github.com/Hamster-Park
(privated)

Miro : https://miro.com/app/board/uXjVNCBP3XY=/?share_link_id=267515060703
